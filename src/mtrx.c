...
double in_prod(VEC *a, VEC *b) { /* a'b */
	...
	if (! gl_blas) {
    	...
    	#else
    	    return cblas_ddot(a->dim, a->ve, one, b->ve, one);
    	#endif
	}
}
...
VEC *vm_mlt(MAT *m, VEC *v, VEC *out) { /* out <- v m */
	...
	if (! gl_blas) {
        ...
        #else
    	    cblas_dgemv(CblasColMajor, CblasTrans, m->m, m->n, alpha, m->v,
                    m->m, v->ve, one, beta, out->ve, one);
    	#endif
	}
	return(out);
}
	...
	if (! gl_blas) {
		...
		int one = 1;
		    ...
		    F77_CALL(dgemv)("N", (int *) &(m->m), (int *) &(m->n), &alpha, m->v,
			    (int *) &(m->m), v->ve, &one, &beta, out->ve, &one FCONE);
        #else
            cblas_dgemv(CblasColMajor, CblasNoTrans, m->m, m->n, alpha, m->v,
                         m->m, v->ve, one, beta, out->ve, one);
    	#endif
...
MAT *m_mlt(MAT *m1, MAT *m2, MAT *out) { /* out <- m1 %*% m2 */
	...
	if (! gl_blas) {
        ...
        #else
            cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m1->m, m2->n, (m1->n), alpha,
                        m1->v, (m1->m),
                        m2->v, (m2->m),
                        beta, out->v, m1->m);;
    	#endif
...
MAT *mtrm_mlt(MAT *m1, MAT *m2, MAT *out) { /* out <- t(m1) %*% m2 */
	...
	if (! gl_blas) {
        ...
        #else
            cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, m1->n, m2->n, m1->m, alpha,
                        m1->v, m1->m,
                        m2->v, m2->m,
                        beta, out->v, m1->n);
        #endif
	...
	if (! gl_blas) {
		...
		double alpha = 1.0, beta = 0.0;
            ...
            F77_CALL(dgemm)("N", "T", (int *) &(m1->m), (int *) &(m2->m), (int *) &(m1->n), &alpha,
                ...
                &beta, out->v, (int *) &(m1->m) FCONE FCONE);
        #else
            cblas_dgemm(CblasColMajor,CblasNoTrans, CblasTrans, (m1->m), (m2->m), (m1->n), alpha,
                    m1->v, (m1->m),
                    m2->v, (m2->m),
                    beta, out->v, (m1->m));
        #endif
...
MAT *CHfactor(MAT *m, PERM *piv, int *info) {
	...
	if (piv == PNULL) { /* Choleski: */
        ...
        #else
            *info = LAPACKE_dpotrf(LAPACK_COL_MAJOR, 'U', (m->n), m->v, (m->n) );
        #endif
		if (*info != 0) {
	    	if (*info > 0 && DEBUG_COV)
		...
		int lwork = -1;
            ...
            efree(work);
		#else
            *info  = LAPACKE_dsytrf(LAPACK_COL_MAJOR, 'U', (m->n), m->v,(m->n), (int *) piv->pe);
            *info  = LAPACKE_dsytrf(LAPACK_COL_MAJOR, 'U', (m->n), m->v, (m->n), (int *) piv->pe);
        #endif
...
MAT *CHsolve(MAT *m, MAT *b, MAT *out, PERM *piv) { /* solve A X = B after factorizing A */
	...
	if (piv == PNULL) /* Choleski */
		...
		#else
		    info = LAPACKE_dpotrs(LAPACK_COL_MAJOR, 'U',  (m->m), (b->n), m->v,(m->m),out->v, (m->m));
		#endif
	else /* LDL' */
        #ifndef CPP_STANDALONE
		    F77_CALL(dsytrs)("Upper", (int *) &(m->m), (int *) &(b->n), m->v, (int *) &(m->m), piv->pe, out->v, (int *) &(m->m), &info, (FC_LEN_T) 5);
		#else
		    info = LAPACKE_dsytrs(LAPACK_COL_MAJOR, 'U', m->m, b->n, m->v,m->m, piv->pe, out->v, m->m);
		#endif
	if (info < 0)
...
VEC *CHsolve1(MAT *m, VEC *b, VEC *out, PERM *piv) { /* solve A x = b after factorizing A */
	...
	if (piv == PNULL)
        ...
        #else
            info = LAPACKE_dpotrs(LAPACK_COL_MAJOR, 'U', m->m, one, m->v, m->m, out->ve, m->m);
        #endif
	else
        #ifndef CPP_STANDALONE
		    F77_CALL(dsytrs)("L", (int *) &(m->m), (int *) &one, m->v, (int *) &(m->m), piv->pe, out->ve, (int *) &(m->m), &info FCONE);
        #else
            info = LAPACKE_dsytrs(LAPACK_COL_MAJOR, 'L', m->m, one, m->v, m->m, piv->pe, out->ve, m->m);
        #endif
	if (info < 0)
